...
##					QMessageBox.warning(self, "Errore", str(e),  QMessageBox.Ok)
	...
	def on_pushButton_search_go_pressed(self):
		if self.BROWSE_STATUS != "f":
			...
			self.TABLE_FIELDS[34] : "'"+str(self.comboBox_funz_statica_usm.currentText())+"'",									#24 - order layer
			...
			self.TABLE_FIELDS[44] : "'"+str(self.comboBox_consistenza_legante_usm.currentText())+"'",									#24 - order layer
			self.TABLE_FIELDS[45] : "'"+str(self.comboBox_colore_legante_usm.currentText())+"'",									#24 - order layer
			self.TABLE_FIELDS[46] : "'"+str(self.lineEdit_aggregati_legante_usm.text())+"'",
			self.TABLE_FIELDS[47] : "'"+str(self.comboBox_consistenza_texture_mat_usm.currentText())+"'",									#24 - order layer
			self.TABLE_FIELDS[48] : "'"+str(self.comboBox_colore_materiale_usm.currentText())+"'"								#24 - order layer
...
##			eval(cmd)
	...
	def fill_fields(self, n=0):
		...
		try:
			...
			str(self.comboBox_conservazione.setEditText(self.DATA_LIST[self.rec_num].stato_di_conservazione))			#23 - conservazione
			...
			str(self.comboBox_consistenza_legante_usm.setEditText(self.DATA_LIST[self.rec_num].cons_legante))									#24 - order layer
			str(self.comboBox_colore_legante_usm.setEditText(self.DATA_LIST[self.rec_num].col_legante))						#24 - order layer
			str(self.lineEdit_aggregati_legante_usm.setText(self.DATA_LIST[self.rec_num].aggreg_legante))
			str(self.comboBox_consistenza_texture_mat_usm.setEditText(self.DATA_LIST[self.rec_num].con_text_mat))									#24 - order layer
			str(self.comboBox_colore_materiale_usm.setEditText(self.DATA_LIST[self.rec_num].col_materiale))							#24 - order layer
