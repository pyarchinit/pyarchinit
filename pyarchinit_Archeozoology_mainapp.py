...
class pyarchinit_Archeozoology(QDialog, Ui_DialogArcheoZoology):
    ...
    def on_calcola_pressed(self):  # ####modifiche apportate per il calcolo statistico con R
        ...
        n = \
            'r(\'con <- dbConnect(drv, host="%s", dbname="%s", port="%d", password="%s", user="%s")\')' \
        ...
        if self.canidi.isChecked() == True:
            ...
            pass
            c = \
                'r(\'A3 <- gstat(VGM_PARAM_A3, fill.all=TRUE, model=vgm(%d,"%s",%s, nugget = %d, kappa= %s))\')' \
                % (int(self.psill.text()),
    ...
    def on_mappa_pressed(self):  # ####modifiche apportate per il calcolo statistico con R
        ...
        if self.canidi.isChecked() == True:
            ...
            pass
            c = \
                'r(\'VGM_PARAM_A3 <- gstat(VGM_PARAM_A3, fill.all=TRUE, model=vgm(%d,"%s",%s, nugget = %d, kappa= %s))\')' \
                % (int(self.psill.text()),
                   str(self.model.currentText()),
                   str(self.rang.text()), int(self.nugget_2.text()),
                   str(self.kappa.text()))
    ...
    def on_automap_pressed(self):  # ####modifiche apportate per il calcolo statistico con R
        ...
        kr = \
            'r(\'kriging_result = autoKrige(%s~1, archezoology_table,model=c("%s"),fix.values = c(%s,%s,%s))\')' \
...
''')
        error = \
            "r('write.table(ME(verifica_errore),\"%s\ME.txt\");write.table(MSE(verifica_errore),\"%s\MSE.txt\");write.table(MSDR(verifica_errore),\"%s\MSDR.txt\")')" \
            % (str(self.lineEdit_esp_generale.text()),
               str(self.lineEdit_esp_generale.text()),
               str(self.lineEdit_esp_generale.text()))
...
title(sub="Rosso = coppie con r>|0.5|, Verde = coppie con |0.25|<r<|0.5|;
    ...
    def on_pushButton_search_go_pressed(self):
        if self.BROWSE_STATUS != 'f':
            ...
            search_dict = {  # 1 - Sito
                ...
                self.TABLE_FIELDS[7]: bos_bison,
                self.TABLE_FIELDS[8]: calcinati,
                self.TABLE_FIELDS[9]: camoscio,
                self.TABLE_FIELDS[10]: capriolo,
                self.TABLE_FIELDS[11]: cervo,
                self.TABLE_FIELDS[12]: combusto,
                self.TABLE_FIELDS[13]: coni,
                self.TABLE_FIELDS[14]: pdi,
                self.TABLE_FIELDS[15]: stambecco,
                self.TABLE_FIELDS[16]: strie,
    ...
    def rec_toupdate(self):
        rec_to_update = \
            self.UTILITY.pos_none_in_list(self.DATA_LIST_REC_TEMP)
