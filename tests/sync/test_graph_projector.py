...
def test_projector_recognizes_localized_su_wsu(tmp_path):
    """khutm fix (2026-06): a non-Italian DB using localized US/USM codes
    (English SU/WSU) must build real stratigraphic nodes — get the ``us``
    attribute, create rapporti edges, and emit a localized d13 — not be
    silently skipped (which left 0 edges / no grouping). Mirrors the real
    Al-Khutm data shape (unita_tipo SU/WSU, English rapporti labels)."""
    import shutil
    import sqlite3
