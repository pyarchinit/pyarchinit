...
def SeqStr(obj, head='c(', tail=')', enclose=True):
    ...
    is_int = tp0 in (int, int) # token for explicit converstion to integer in R since R treat an integer from stdin as double
    if tp0 not in simple_types:
        head = 'list('
    else:
        tps = isinstance(obj0, str) and [StringType] or isinstance(obj0, bool) and [BooleanType] or num_types
        for i in obj[1:]:
            tp = type(i)
            if tp not in tps:
                head = 'list('
...
str_func = {type(None): NoneStr, bool: BoolStr, int: LongStr, int: repr, float: FloatStr, complex: ComplexStr, 
        str: UniStr, str: repr, list: SeqStr, tuple: SeqStr, set: SeqStr, frozenset: SeqStr, dict: DictStr} # str will override uncode in Python 3
...
class R(object): # "del r.XXX" fails on FePy-r7 (IronPython 1.1 on .NET 2.0.50727.42) if using old-style class
    ...
    def __call__(self, CMDS=[], use_try=None):
        ...
        '''
        rlt = []
        if isinstance(CMDS, str):  # a single command
