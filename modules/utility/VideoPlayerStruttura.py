...
class VideoPlayerWindow(QMainWindow):
    ...
    def save_frame_to_db(self):
        ...
        sigla = self.mainclass.comboBox_sigla_struttura.currentText()
        nv = self.mainclass.numero_struttura.text()

        if not sito or not nv or not sigla:
    ...
    def generate_US(self):
        ...
        records = self.DB_MANAGER.query_bool(search_dict, 'STRUTTURA')

        us_list = []
        for record in records:
            if hasattr(record, 'id_struttura'):
                us_list.append([record.id_struttura, 'STRUTTURA', 'struttura_table'])
            else:
