...
from sqlalchemy import Table, Column, Integer, String, Float, ForeignKey, MetaData, create_engine, Text
...
class Tma_materiali_table:
    ...
    # First ensure the parent table is defined in the same metadata
    tma_table = Table('tma_materiali_archeologici', metadata,
                      Column('id', Integer, primary_key=True),
                      # Only define the minimum necessary columns
                      extend_existing=True  # Don't redefine if already exists
                     )
        
    tma_materiali_table = Table('tma_materiali_ripetibili', metadata,
                     Column('id', Integer, primary_key=True),
                     
                     # Foreign key to main TMA record - remove explicit reference
                     Column('id_tma', Integer, nullable=False),  # No ForeignKey definition
