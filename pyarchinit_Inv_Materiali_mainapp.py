...
class pyarchinit_Inventario_reperti(QDialog, Ui_DialogInventarioMateriali):
    ...
    def loadMediaPreview(self, mode=0):
        ...
        if mode == 0:
            ...
            rec_list = self.ID_TABLE + " = " + str(
                ast.literal_eval("self.DATA_LIST[int(self.REC_CORR)]." + self.ID_TABLE))
            search_dict = {
                'id_entity': "'" + str(ast.literal_eval("self.DATA_LIST[int(self.REC_CORR)]." + self.ID_TABLE)) + "'",
                'entity_type': "'REPERTO'"}
    ...
    def on_toolButtonPreviewMedia_toggled(self):
        if self.toolButtonPreviewMedia.isChecked():
            QMessageBox.warning(self, "Messaggio",
                                "Modalita' Preview Media Reperti attivata. Le immagini dei Reperti saranno visualizzate nella sezione Media",
                                QMessageBox.Ok)
        ...
        for cassa in elenco_casse_list:
            ...
            for i in range(len(res_inv_list)):
                if i != len(res_inv_list) - 1:
                    n_inv_res_list += "N.inv:" + str(res_inv_list[i].numero_inventario) + "/" + str(
    ...
    def update_tot_frammenti(self, c):
        ...
        if self.choice == 1:
            ...
            search_dict = {
                'id_invmat': "'" + str(ast.literal_eval("self.DATA_LIST[int(self.REC_CORR)]." + self.ID_TABLE)) + "'"}
