...
from modules.gui.pyarchinit_inventario_reperti_ui import Ui_DialogInventarioMateriali
...
if bool(dataset):
	...
	def parameter_quant_creator(self, par_list, n_rec):
		...
		for sing_par_conv in range(len(converted_parameters)):
			...
			exec_str = ' -' + paramentro[:4] + ": " + ast.literal_eval(exec_str)
			parameter2 += exec_str
		return parameter2
...
if bool(self.DATA_LIST):
	...
	def loadMediaPreview(self, mode = 0):
		...
		if mode == 0:
			...
			search_dict = {'id_entity'  : "'"+str(ast.literal_eval("self.DATA_LIST[int(self.REC_CORR)]." + self.ID_TABLE))+"'", 'entity_type' : "'REPERTO'"}
...
if not bool(res):
	...
	def update_tot_frammenti(self, c):
        ...
        if bool(elementi_reperto):
			...
			search_dict = {'id_invmat'  : "'"+str(ast.literal_eval("self.DATA_LIST[int(self.REC_CORR)]." + self.ID_TABLE))+"'"}
...
if bool(sub_list):
	...
	def fill_fields(self, n=0):
		...
		try:
			...
			str(self.comboBox_criterio_schedatura.setEditText(self.DATA_LIST[self.rec_num].criterio_schedatura))		#4 - Criterio schedatura
			...
			str(self.lineEdit_luogo_conservazione.setText(self.DATA_LIST[self.rec_num].luogo_conservazione))			#11 - luogo_conservazione

			self.comboBox_conservazione.setEditText(str(self.DATA_LIST[self.rec_num].stato_conservazione))				#12 - stato conservazione

			str(self.lineEdit_datazione_rep.setText(self.DATA_LIST[self.rec_num].datazione_reperto))					#13 - datazione reperto
