...
CORRECTED VERSION: Uses 10.3 instead of 10.18 for località
"""
...
def insert_localita_values(cursor):
    ...
    # Prima rimuovi i valori località esistenti (both old 10.18 and correct 10.3)
    cursor.execute("""
        DELETE FROM pyarchinit_thesaurus_sigle 
        WHERE nome_tabella = 'TMA materiali archeologici' 
        AND tipologia_sigla IN ('10.18', '10.3')
    """)
    print("✓ Rimossi valori località esistenti")
    ...
    print("\nInserimento valori località (10.3):")
    inserted = 0
    ...
    for id_val, localita in localita_values:
        ...
        cursor.execute("""
            ...
            (id_thesaurus_sigle, nome_tabella, sigla, sigla_estesa, tipologia_sigla, lingua, hierarchy_level)
            VALUES (?, 'TMA materiali ripetibili', ?, ?, '10.3', 'it', 1)
        """, (id_thesaurus, sigla, localita))
