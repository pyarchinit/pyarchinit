...
# -*- coding: utf-8 -*-
"""
Script per inserire i valori dtzg (Fascia cronologica di riferimento) dall'Excel
"""
...
def insert_dtzg_values(cursor):
    ...
    print("\nInserimento valori dtzg (Fascia cronologica - 10.10):")
    inserted = 0
    
    for id_val, fascia, cronologia in dtzg_values:
        # Usa DTZG + numero come sigla
        sigla = f"DTZG{id_val:02d}"
        
        # Combina fascia e cronologia per sigla_estesa
        if cronologia:
            sigla_estesa = f"{fascia} ({cronologia})"
        else:
            sigla_estesa = fascia
        ...
        if inserted < 15 or inserted % 10 == 0:  # Mostra solo alcuni per non inondare l'output
            print(f"  ✓ {id_val}. {fascia}" + (f" ({cronologia})" if cronologia else ""))
        inserted += 1
    
    print(f"  ... (totale {inserted} fasce cronologiche inserite)")
...
def main():
    ...
    try:
        print("Inserimento valori dtzg (Fascia cronologica) dall'Excel...")
        print("=" * 60)
        
        inserted = insert_dtzg_values(cursor)
        ...
        cursor.execute("""
            ...
            WHERE tipologia_sigla = '10.10' AND sigla_estesa LIKE '%Neolitico%'
        """)
        ...
        cursor.execute("""
            SELECT COUNT(*) 
            FROM pyarchinit_thesaurus_sigle 
            WHERE tipologia_sigla = '10.10' AND sigla_estesa LIKE '%Minoico%'
        """)
