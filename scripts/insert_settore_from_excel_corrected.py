...
CORRECTED VERSION: Uses 10.15 instead of 10.19 for settore
"""
...
def insert_settore_values(cursor):
    ...
    # Prima rimuovi i valori settore esistenti (both old 10.19 and correct 10.15)
    cursor.execute("""
        DELETE FROM pyarchinit_thesaurus_sigle 
        WHERE nome_tabella = 'TMA materiali archeologici' 
        AND tipologia_sigla IN ('10.19', '10.15')
    """)
    print("✓ Rimossi valori settore esistenti")
    ...
    print("\nInserimento valori settore (10.15) con relazioni:")
    inserted = 0
    ...
    # Also insert for TMA materiali ripetibili
    for id_settore, id_localita, id_area, settore_nome in settore_values:
        sigla = f"SETT{id_settore:03d}"
        id_thesaurus = 3000 + id_settore
        id_parent = 2000 + id_area
        ...
        cursor.execute("""
            ...
            (id_thesaurus_sigle, nome_tabella, sigla, sigla_estesa, tipologia_sigla, lingua, 
             id_parent, parent_sigla, hierarchy_level)
            VALUES (?, 'TMA materiali ripetibili', ?, ?, '10.15', 'it', ?, ?, 3)
        """, (id_thesaurus, sigla, settore_nome, id_parent, parent_sigla))
    
    return inserted

def main():
    # Database path - use the actual plugin database
    ...
    try:
        ...
        cursor.execute("""
            ...
            AND nome_tabella = 'TMA materiali archeologici'
