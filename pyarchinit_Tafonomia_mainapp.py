...
class pyarchinit_Tafonomia(QDialog, Ui_Dialog_tafonomia):
	...
	def on_pushButton_new_search_pressed(self):
		if self.check_record_state() == 1:
			...
			if self.BROWSE_STATUS != "f":
				...
				self.setTableEnable(["self.tableWidget_caratteristiche", "self.tableWidget_corredo_tipo", "self.tableWidget_misurazioni"], "False")
	...
	def on_pushButton_search_go_pressed(self):
		if self.BROWSE_STATUS != "f":
			...
			self.TABLE_FIELDS[6]  : "'"+str(self.textEdit_descrizione_taf.toPlainText())+"'",						#7 - Descrizione tafonimia
			self.TABLE_FIELDS[7]  : "'"+str(self.textEdit_interpretazione_taf.toPlainText())+"'",					#8 - Interpretazione tafonimia
			...
			if not bool(search_dict):
				...
				if not bool(res):
					...
					self.setTableEnable(["self.tableWidget_caratteristiche", "self.tableWidget_corredo_tipo","self.tableWidget_misurazioni"], "True")
				else:
					self.DATA_LIST = []
					for i in res:
						self.DATA_LIST.append(i)
	...
	def fill_fields(self, n=0):
		...
		if bool(self.DATA_LIST):
			try:
				...
				str(self.textEdit_descrizione_taf.setText(self.DATA_LIST[self.rec_num].descrizione_taf))									#7 - descrizione_taf
				str(self.textEdit_interpretazione_taf.setText(self.DATA_LIST[self.rec_num].interpretazione_taf))							#8 - interpretazione_taf
				...
				str(self.textEdit_descrizione_corredo.setText(self.DATA_LIST[self.rec_num].corredo_descrizione))							#17 - descrizione corredo
				...
				self.tableInsertData("self.tableWidget_misurazioni", self.DATA_LIST[self.rec_num].misure_tafonomia)							#28 - misure struttura
