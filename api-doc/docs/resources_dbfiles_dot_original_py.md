# resources/dbfiles/dot_original.py
...
Initializes a new `Node` instance with default values for all instance attributes. Sets `label` to an empty string, `id` to `0`, `attribs` to an empty dictionary, `referenced` to `False`, and `sections` to an empty list. This constructor takes no arguments beyond the implicit `self` reference.
...
##### exportGraphml(self, doc, parent, conf)
...
Initializes a new `Edge` instance with default values for all instance attributes. Sets `id` to `0`, `src` and `dest` to empty strings, and `attribs` to an empty dictionary.
