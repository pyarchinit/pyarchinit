# resources/dbfiles/dot_backup.py
...
Iterates over the `epoch` list and checks whether each element is present in `nome_us`, assigning `y_value` as the element's index multiplied by `1000` whenever a match is found. The final value of `y_value` reflects the last matching index position scaled by `1000`; if no match is found, `0` is returned. Returns the computed integer `y_value`.
