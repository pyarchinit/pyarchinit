...
class pyarchinit_Schedaind(QDialog, MAIN_DIALOG_CLASS):
    ...
    def charge_list(self):
        ...
        # lista stima del sesso

        self.comboBox_sesso.clear()
        search_dict = {
            'nome_tabella': "'" + 'individui_table' + "'",
            'tipologia_sigla': "'" + 'sesso' + "'"
        }

        sesso = self.DB_MANAGER.query_bool(search_dict, 'PYARCHINIT_THESAURUS_SIGLE')
        sesso_vl = []

        for i in range(len(sesso)):
            sesso_vl.append(sesso[i].sigla_estesa)

        sesso_vl.sort()
        self.comboBox_sesso.addItems(sesso_vl)

        # lista classi eta

        self.comboBox_classi_eta.clear()
        search_dict = {
            'nome_tabella': "'" + 'individui_table' + "'",
            'tipologia_sigla': "'" + 'classi eta' + "'"
        }

        classi_eta = self.DB_MANAGER.query_bool(search_dict, 'PYARCHINIT_THESAURUS_SIGLE')
        classi_eta_vl = []

        for i in range(len(classi_eta)):
            classi_eta_vl.append(classi_eta[i].sigla_estesa)

        classi_eta_vl.sort()
        self.comboBox_classi_eta.addItems(classi_eta_vl)

        # lista eta min

        self.comboBox_eta_min.clear()
        search_dict = {
            'nome_tabella': "'" + 'individui_table' + "'",
            'tipologia_sigla': "'" + 'eta min' + "'"
        }

        eta_min = self.DB_MANAGER.query_bool(search_dict, 'PYARCHINIT_THESAURUS_SIGLE')
        eta_min_vl = []

        for i in range(len(eta_min)):
            eta_min_vl.append(eta_min[i].sigla_estesa)

        eta_min_vl.sort()
        self.comboBox_eta_min.addItems(eta_min_vl)

        # lista eta max

        self.comboBox_eta_max.clear()
        search_dict = {
            'nome_tabella': "'" + 'individui_table' + "'",
            'tipologia_sigla': "'" + 'eta max' + "'"
        }

        eta_max = self.DB_MANAGER.query_bool(search_dict, 'PYARCHINIT_THESAURUS_SIGLE')
        eta_max_vl = []

        for i in range(len(eta_max)):
            eta_max_vl.append(eta_max[i].sigla_estesa)

        eta_max_vl.sort()
        self.comboBox_eta_max.addItems(eta_max_vl)
