    ...
    def on_pushButton_print_pressed(self):
        if self.L=='it':
            if self.checkBox_s_us.isChecked():
                ...
                QMessageBox.warning(self, 'Ok',"Esportazione terminata Schede Individui",QMessageBox.StandardButton.Ok)
            else:   
                pass
            if self.checkBox_e_us.isChecked() :
                ...
                try:               
                    if bool(data_list):
                        ...
                        QMessageBox.warning(self, 'Ok',"Esportazione terminata Elenco Individui",QMessageBox.StandardButton.Ok)
                    else:
                        QMessageBox.warning(self, 'ATTENZIONE',"L'elenco Individui non può essere esportato devi riempire prima le schede Individui",QMessageBox.StandardButton.Ok)
                except Exception as e :
            ...
            if self.checkBox_e_us.isChecked() :
                ...
                try:               
                    if bool(data_list):
                        ...
                        QMessageBox.warning(self, 'WARNING',"The SU list cannot be exported you have to fill in the SU tabs first",QMessageBox.StandardButton.Ok)
                except Exception as e :
            ...
            if self.checkBox_e_foto.isChecked():
                ...
                try:
                        if bool(data_list_foto):
                            US_index_pdf.build_index_Foto_2(data_list_foto, data_list_foto[0][0])
                            QMessageBox.warning(self, 'Ok', "Export finished Photo List without thumbanil",QMessageBox.StandardButton.Ok)
                        else:
                            QMessageBox.warning(self, 'WARNING', "The photo list cannot be exported because you do not have tagged images.",QMessageBox.StandardButton.Ok)
                except Exception as e :
                    QMessageBox.warning(self, 'WARNING',str(e),QMessageBox.StandardButton.Ok)
            ...
            if self.checkBox_e_us.isChecked() :
                ...
                try:               
                    if bool(data_list):
                        ...
                        QMessageBox.warning(self, 'WARNUNG', 'Die Liste kann nicht exportiert werden, Sie müssen zuerst die Formulare ausfüllen',QMessageBox.StandardButton.Ok)
                except Exception as e :
