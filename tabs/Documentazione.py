...
class pyarchinit_Documentazione(QDialog, MAIN_DIALOG_CLASS):
    ...
    def on_pushButtonPreview_pressed(self):
        ...
        QMessageBox.warning(self, "query", str(docstr), QMessageBox.StandardButton.Ok)
        ...
        if hasattr(self, 'concurrency_manager'):
            try:
                if n < len(self.DATA_LIST):
                    ...
                    if hasattr(current_record, 'id_documentazione'):
                        self.editing_record_id = getattr(current_record, 'id_documentazione')
