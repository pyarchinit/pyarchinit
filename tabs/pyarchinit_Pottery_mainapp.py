...
class pyarchinit_Pottery(QDialog, MAIN_DIALOG_CLASS):
    ...
    def on_pushButton_all_images_pressed(self):
        ...
        for i in record_us_list:
            ...
            search_dict = {'id_media': "'" + str(i.id_media) + "'", 'entity_type': "'CERAMICA'"}
            search_dict = u.remove_empty_items_fr_dict(search_dict)
            ...
            if mediatoentity_data:
                ...
                if us_data:
                    item.setText(item.text() + " - Pottery: " + str(us_data[0].nr_scheda_taf))
                else:
                    item.setText(item.text() + " - Pottery: Non trovato")
