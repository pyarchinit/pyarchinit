...
class AnimatedGearWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._target_speed_deg_s = 42.0  # degrees/sec for gear 1
        self._tau_s = 0.25  # inertia time constant (sec)
    ...
    def paintEvent(self, event):
        ...
        # Compute gear sizes so they mesh and fit the widget
        min_dim = float(min(self.width(), self.height()))
        r_sum = min_dim * 0.30  # sum of pitch radii
        ...
        # Draw gears (behind the logo)
        self._draw_gear(painter, g1x, gy, r1_root, r1_outer, self.teeth1, self.angle1, self.color1)
        self._draw_gear(painter, g2x, gy, r2_root, r2_outer, self.teeth2, self.angle2, self.color2)

        # Draw the logo in the center (leave room so teeth remain visible)
        if self.logo:
            logo_size = int(min(self.width(), self.height()) * 0.38)
            scaled_logo = self.logo.scaled(logo_size, logo_size,
                                            Qt.AspectRatioMode.KeepAspectRatio,
                                            Qt.TransformationMode.SmoothTransformation)
            logo_x = int(cx - scaled_logo.width() * 0.5)
            logo_y = int(gy - scaled_logo.height() * 0.5)
    ...
    def _draw_gear(self, painter, cx, cy, r_root, r_outer, teeth, angle_deg, base_color):
        """Draw a shaded gear with squared teeth."""
        ...
        # Hub + bore
        hub_r = (r_root + r_outer) * 0.40
        bore_r = (r_root + r_outer) * 0.20

        hub_grad = QRadialGradient(QPointF(-hub_r * 0.25, -hub_r * 0.25), hub_r)
        hub_grad.setColorAt(0.0, QColor(255, 255, 255, 190))
        hub_grad.setColorAt(0.6, QColor(c).lighter(130))
        hub_grad.setColorAt(1.0, QColor(c).darker(175))

        painter.setPen(QPen(QColor(0, 0, 0, 60), 1.0))
        painter.setBrush(QBrush(hub_grad))
        painter.drawEllipse(QPointF(0, 0), hub_r, hub_r)

        painter.setPen(QPen(QColor(0, 0, 0, 95), 1.2))
        painter.setBrush(QBrush(QColor(20, 20, 20, 210)))
        painter.drawEllipse(QPointF(0, 0), bore_r, bore_r)
