...
> File: `modules/utility/matrix_layout_policy.py`, `modules/utility/pyarchinit_matrix_exp.py`, `tests/utility/test_matrix_layout_policy.py`, `modules/s3dgraphy/sync/graph_projector.py`, `tests/sync/test_graph_projector.py`.

### Italiano

#### Contesto

- Stesso DB utente del fix 2026-08-26 (`SanGiovanniinMarignano_RN_Invaso_Ventena_FINALE.sqlite`, 1311 US, 2039 archi) ma con periodizzazione compilata. Due malfunzionamenti distinti: "Esporta matrix con periodizzazione" (`HarrisMatrix.export_matrix` con `checkBox_period` attivo) abortiva, e l'Extended Matrix GraphML esportato via s3dgraphy mostrava paragrafi interi come etichetta delle righe epoca.

#### 1. Export matrix con periodizzazione (`64d65108`)

- **Sintomo:** dopo ~25 s `Errore durante il rendering del file DOT: 'NoneType' object has no attribute 'write'`, nessun file prodotto.
- **Causa radice:** graphviz-python (`graphviz.backend.execute.run_check` → `_write_stderr`) rilancia con `sys.stderr.write()` tutto ciò che `dot` stampa su stderr; in un python GUI senza console (QGIS su Windows prima che venga aperta la console Python, `pythonw`) `sys.stderr` è `None`. L'export con periodizzazione era l'UNICO percorso in cui dot emette un warning — `Warning: Two clusters named cluster_cont - the second will be ignored` — perché il cluster invisibile dei nodi contemporanei si chiamava `cluster_cont` dentro OGNI cluster di fase. Riprodotto end-to-end nel python di QGIS con `sys.stderr = None`: su HEAD l'errore identico dopo 25 s; con il fix 2,4 s e JPG valido.
- **Fix in `modules/utility/matrix_layout_policy.py` e `modules/utility/pyarchinit_matrix_exp.py`:**
  - **`graphviz_stderr_guard(label='graphviz')`** (NUOVO context manager, policy module): redirige stderr in un `io.StringIO` (`contextlib.redirect_stderr`) per la durata del `render()`, poi rilancia il testo con `print(f"{label}: {text}")` (no-op se anche stdout è `None`); il buffer viene yielded. In `pyarchinit_matrix_exp.py` nuovo helper di modulo **`_render(graph, **kw)`** = `graph.render(**kw)` dentro il guard; TUTTE le chiamate `render()` passano da lì: `HarrisMatrix.export_matrix` (DOT + doppio tentativo JPG), `HarrisMatrix.export_matrix_2`, `ViewHarrisMatrix.export_matrix` (DOT, PNG, JPG), `ViewHarrisMatrix.export_matrix_3` (anche dentro il ciclo di retry sul dpi, con `cleanup=True`) e `_render_vector_copies`.
  - **Cluster dei contemporanei nominato per fase:** `f.subgraph(name=f'{fase_key}_cont')` al posto di `'cluster_cont'` (in `HarrisMatrix.export_matrix` e `ViewHarrisMatrix.export_matrix_3`) → nessun warning di nome duplicato e dot non ignora più dal 2° cluster in poi (prima solo il primo veniva realmente applicato).
  - **Archi duplicati O(n²):** i sottografi `main1` (archi contemporanei) e `main2` (archi negativi) venivano riaperti DENTRO il ciclo per-arco (`for cc in self.contemporary: … with G.subgraph(name='main1') as b: b.edges(elist3)`), riemettendo a ogni iterazione la lista che cresce: 225 576 righe di arco per 2 039 rapporti, layout da 51 MB, 25–44 s di `dot`. I due `with G.subgraph(...)` sono stati portati fuori dai cicli (un solo blocco dopo ciascun `for`) sia in `export_matrix` sia in `export_matrix_3` → sorgente DOT 122 KB, layout 634 KB, 2,4 s. Attributi nodo/arco invariati.
  - **`bb` in notazione esponenziale:** per layout molto larghi dot scrive il `bb` radice come `bb="0,0,2.5335e+05,7785"` (253 350 pt ≈ 88 m); la regex `_BB_RE` di `safe_raster_dpi` (`[-\d.]+`) non lo riconosceva → nessun clamp del dpi → cairo rifiutava la bitmap e il JPG usciva da 0 byte. Nuovo pattern `_NUM = r'[-+]?(?:\d+\.?\d*|\.\d+)(?:[eE][-+]?\d+)?'` per le quattro coordinate; **`MIN_RASTER_DPI` abbassato da 12 a 1** perché a 12 dpi 253 350 pt danno ancora 42 225 px > 32 767: il raster resta così sempre entro il limite cairo (a dpi bassissimi è solo un'anteprima, le copie `.svg`/`.pdf` sono l'output leggibile). Sul DB reale, con tutti i fix, la matrix con periodizzazione è resa a 49 dpi, 30792×5414 px, più `.svg`/`.pdf` accanto.

#### 2. GraphML Extended Matrix: nome delle righe epoca (`a1737a5e`)

- **Sintomo:** nel GraphML esportato dal DB Ventena ogni riga delle swimlane epoca (yEd) mostrava un intero paragrafo — la `descrizione` di `periodizzazione_table` — invece del nome del periodo.
- **Causa radice (`modules/s3dgraphy/sync/graph_projector.py`, `GraphProjector._enrich_into`):** ogni `EpochNode` veniva nominato con `descrizione` (`label = descr or f"Period {P} Phase {F}"`) e s3dgraphy stampa `EpochNode.name` su ogni riga (`epoch_generator.py`: `f"{epoch.name} [start:..;end:..]"`). Nel DB Ventena `descrizione` contiene paragrafi e il nome del periodo sta in `datazione_estesa`. Mai notato prima perché nei DB di esempio `descrizione` == `datazione_estesa` ("Età contemporanea"). In più `attributes['datazione_estesa']` conteneva in realtà `descrizione`.
- **Fix:** la query legge anche `datazione_estesa` (`SELECT periodo, fase, cron_iniziale, cron_finale, descrizione, datazione_estesa FROM periodizzazione_table`), con fallback alla query a 5 colonne (righe completate con `(None,)`) per gli schemi vecchi senza la colonna. `EpochNode.name` ← `datazione_estesa` (lo stesso campo che l'export DOT con periodizzazione usa per l'etichetta del cluster), fallback `descrizione`, poi `"Period P Phase F"`; `descrizione` → `EpochNode(description=…)`; `attributes['datazione_estesa']` porta la vera `datazione_estesa`. Nomi resi unici quando più fasi condividono la stessa `datazione_estesa` (pre-pass `base_labels` + `collections.Counter`, suffisso `" (periodo P, fase F)"` solo in caso di collisione — nel DB Ventena tre fasi del periodo 4): necessario perché il round-trip yEd reidrata periodo/fase sugli `EpochNode` per nome (`pyarchinit.epochs_meta` in `graph_ingestor.py`) e yEd non mostra altro sulla riga.

#### Test

- **`tests/utility/test_matrix_layout_policy.py` (+2, 7 totali):** `test_bb_in_exponent_notation_is_parsed_and_clamped` (`bb="0,0,2.5335e+05,7785"` → `laid_out_size_points` = `(253350.0, 7785.0)`, dpi tale che la larghezza ≤ 32 000 px e ≥ 1); `test_stderr_guard_makes_graphviz_render_survive_sys_stderr_none` (`pytest.importorskip("graphviz")`; nodo `fixedsize` troppo piccolo per la label → dot stampa un Warning; con `sys.stderr = None` il `render()` nudo alza `AttributeError`, dentro `graphviz_stderr_guard` scrive il file e il warning finisce nel buffer). Il test del floor confronta ora con `MIN_RASTER_DPI`.
- **`tests/sync/test_graph_projector.py::test_projector_epoch_name_is_datazione_estesa_not_descrizione` (NUOVO):** DB SQLite con 5 righe di periodizzazione (paragrafo lungo + `datazione_estesa`; `datazione_estesa` vuota → fallback a `descrizione`; entrambe vuote → `"Period 6 Phase 1"`; due fasi con la stessa `datazione_estesa` → `"Tra fine Età del Ferro (periodo 4, fase 1)"` / `(… fase 2)`), controllo di `name`, `description`, `attributes['datazione_estesa']` e unicità dei nomi. 9/9 nel file; `tests/sync` non-PG: 474 passed.

#### Note per master

- Il fix 1 (stessi due file `matrix_layout_policy.py` + `pyarchinit_matrix_exp.py`) è stato portato su `master` sul branch locale `fix/large-relations-master` (commit `1992e1c1`, base `26adb90b` = v4.9.11), **non ancora pushato né rilasciato**. Il fix 2 riguarda solo il bridge s3dgraphy (`modules/s3dgraphy/` non esiste su `master`).

### English

#### Context

- Same user DB as the 2026-08-26 fix (`SanGiovanniinMarignano_RN_Invaso_Ventena_FINALE.sqlite`, 1311 US, 2039 edges) but with the periodisation filled in. Two separate failures: "Esporta matrix con periodizzazione" (`HarrisMatrix.export_matrix` with `checkBox_period` on) aborted, and the Extended Matrix GraphML exported through s3dgraphy showed whole paragraphs as the label of the epoch rows.

#### 1. Period matrix export (`64d65108`)

- **Symptom:** after ~25 s `Errore durante il rendering del file DOT: 'NoneType' object has no attribute 'write'`, no file produced.
- **Root cause:** graphviz-python (`graphviz.backend.execute.run_check` → `_write_stderr`) echoes whatever `dot` prints on stderr via `sys.stderr.write()`; inside a GUI python without a console (QGIS on Windows before the Python console is opened, `pythonw`) `sys.stderr` is `None`. The period export was the ONLY path where dot prints a warning — `Warning: Two clusters named cluster_cont - the second will be ignored` — because the invisible cluster for contemporary nodes was named `cluster_cont` inside EVERY fase cluster. Reproduced end-to-end under the QGIS python with `sys.stderr = None`: on HEAD the exact error after 25 s; with the fix 2.4 s and a valid JPG.
- **Fixes in `modules/utility/matrix_layout_policy.py` and `modules/utility/pyarchinit_matrix_exp.py`:**
  - **`graphviz_stderr_guard(label='graphviz')`** (NEW context manager, policy module): redirects stderr into an `io.StringIO` (`contextlib.redirect_stderr`) for the duration of `render()`, then re-emits the text with `print(f"{label}: {text}")` (a no-op when stdout is `None` too); the buffer is yielded. In `pyarchinit_matrix_exp.py` a new module helper **`_render(graph, **kw)`** = `graph.render(**kw)` inside the guard; EVERY `render()` call goes through it: `HarrisMatrix.export_matrix` (DOT + two JPG attempts), `HarrisMatrix.export_matrix_2`, `ViewHarrisMatrix.export_matrix` (DOT, PNG, JPG), `ViewHarrisMatrix.export_matrix_3` (also inside the dpi-retry loop, with `cleanup=True`) and `_render_vector_copies`.
  - **Contemporary-nodes cluster named per fase:** `f.subgraph(name=f'{fase_key}_cont')` instead of `'cluster_cont'` (in `HarrisMatrix.export_matrix` and `ViewHarrisMatrix.export_matrix_3`) → no duplicate-name warning, and dot no longer ignores the 2nd..nth cluster (previously only the first one was actually applied).
  - **O(n²) duplicate edges:** the `main1` (contemporary edges) and `main2` (negative edges) subgraphs were re-opened INSIDE the per-edge loops (`for cc in self.contemporary: … with G.subgraph(name='main1') as b: b.edges(elist3)`), re-emitting the growing list on every iteration: 225,576 edge lines for 2,039 relations, a 51 MB layout, 25–44 s of `dot`. Both `with G.subgraph(...)` blocks were hoisted out of the loops (one block after each `for`) in `export_matrix` and `export_matrix_3` → 122 KB DOT source, 634 KB layout, 2.4 s. Node/edge attributes unchanged.
  - **`bb` in exponent notation:** for very wide layouts dot writes the root `bb` as `bb="0,0,2.5335e+05,7785"` (253,350 pt ≈ 88 m); the `_BB_RE` regex of `safe_raster_dpi` (`[-\d.]+`) missed it → no dpi clamp → cairo refused the bitmap and the JPG came out as 0 bytes. New pattern `_NUM = r'[-+]?(?:\d+\.?\d*|\.\d+)(?:[eE][-+]?\d+)?'` for the four coordinates; **`MIN_RASTER_DPI` lowered from 12 to 1** because at 12 dpi 253,350 pt still gives 42,225 px > 32,767: the raster now always fits cairo's limit (at very low dpi it is only an overview, the `.svg`/`.pdf` copies are the readable output). On the real DB, with all fixes, the period matrix renders at 49 dpi, 30792×5414 px, plus `.svg`/`.pdf` next to it.

#### 2. Extended Matrix GraphML: epoch row names (`a1737a5e`)

- **Symptom:** in the GraphML exported from the Ventena DB every epoch swimlane row (yEd) showed a whole paragraph — the `descrizione` of `periodizzazione_table` — instead of the period name.
- **Root cause (`modules/s3dgraphy/sync/graph_projector.py`, `GraphProjector._enrich_into`):** every `EpochNode` was named after `descrizione` (`label = descr or f"Period {P} Phase {F}"`) and s3dgraphy prints `EpochNode.name` on each row (`epoch_generator.py`: `f"{epoch.name} [start:..;end:..]"`). On the Ventena DB `descrizione` holds paragraphs and the period name lives in `datazione_estesa`. Never noticed before because in the sample DBs `descrizione` == `datazione_estesa` ("Età contemporanea"). On top of that, `attributes['datazione_estesa']` actually carried `descrizione`.
- **Fix:** the query also reads `datazione_estesa` (`SELECT periodo, fase, cron_iniziale, cron_finale, descrizione, datazione_estesa FROM periodizzazione_table`), falling back to the 5-column query (rows padded with `(None,)`) for older schemas without the column. `EpochNode.name` ← `datazione_estesa` (the same field the DOT period export uses for its cluster label), fallback `descrizione`, then `"Period P Phase F"`; `descrizione` → `EpochNode(description=…)`; `attributes['datazione_estesa']` carries the real `datazione_estesa`. Names are made unique when several phases share one `datazione_estesa` (pre-pass `base_labels` + `collections.Counter`, suffix `" (periodo P, fase F)"` only on collision — three phases of periodo 4 on the Ventena DB): required because the yEd round-trip hydrates periodo/fase onto the `EpochNode`s by name (`pyarchinit.epochs_meta` in `graph_ingestor.py`) and yEd shows nothing else on the row.

#### Tests

- **`tests/utility/test_matrix_layout_policy.py` (+2, 7 total):** `test_bb_in_exponent_notation_is_parsed_and_clamped` (`bb="0,0,2.5335e+05,7785"` → `laid_out_size_points` = `(253350.0, 7785.0)`, dpi such that the width is ≤ 32,000 px and ≥ 1); `test_stderr_guard_makes_graphviz_render_survive_sys_stderr_none` (`pytest.importorskip("graphviz")`; a `fixedsize` node too small for its label → dot prints a Warning; with `sys.stderr = None` the bare `render()` raises `AttributeError`, inside `graphviz_stderr_guard` it writes the file and the warning lands in the buffer). The floor test now compares against `MIN_RASTER_DPI`.
- **`tests/sync/test_graph_projector.py::test_projector_epoch_name_is_datazione_estesa_not_descrizione` (NEW):** SQLite DB with 5 periodisation rows (long paragraph + `datazione_estesa`; empty `datazione_estesa` → fallback to `descrizione`; both empty → `"Period 6 Phase 1"`; two phases sharing one `datazione_estesa` → `"Tra fine Età del Ferro (periodo 4, fase 1)"` / `(… fase 2)`), checking `name`, `description`, `attributes['datazione_estesa']` and name uniqueness. 9/9 in the file; non-PG `tests/sync`: 474 passed.

#### Notes for master

- Fix 1 (same two files `matrix_layout_policy.py` + `pyarchinit_matrix_exp.py`) was ported to `master` on the local branch `fix/large-relations-master` (commit `1992e1c1`, base `26adb90b` = v4.9.11), **not yet pushed nor released**. Fix 2 concerns only the s3dgraphy bridge (`modules/s3dgraphy/` does not exist on `master`).
...
- **Editor per-US "Cronologia & tafonomia"**: il dialog ora **precarica tutte le US del sito** da `us_table` con due colonne informative read-only — **Periodo** (`periodo_iniziale/finale` + fase) e **N. reperti** (conteggio `inventario_materiali` + `pottery` per US) — così assegni il taf a ogni US, non solo a quelle datate. Le US già con cronologia/taf mostrano i valori salvati; al salvataggio si scrivono **solo le US compilate** (con taf e/o data). Tabella a 12 colonne: Sito, Area, US, Periodo(RO), N.rep(RO), taf, C14 BP, ±err, start, end, Lab, source.
- **Colonna `taf [0-1]`** per US: valore tafonomico (0 = reperto del tutto disturbato/redeposto, 1 = integro in posto), **interpretativo**, da assegnare a mano (non calcolato). Persistito in una **nuova colonna `taf`** di `palimpsest_chronology` (migrazione `ALTER TABLE` idempotente per i DB esistenti). Caricato/modificato/salvato come le date (anche per US senza data assoluta: `start`/`end` ora opzionali in *Salva modifiche*); validato in `[0,1]`; supportato anche negli import/export CSV.
...
- **Tutti gli algoritmi onorano il taf**: anche i tre `.rsx` Processing (`r:palimpsestrfit/intrusions/report`) ora leggono il taf per US da `palimpsest_chronology` e sovrascrivono `taf_score` dopo `read_pyarchinit`. Questo è un blocco **pyArchInit-local**: gli `.rsx` non sono più byte-identici a palimpsestr 0.22.0 (la sede pulita resta `read_pyarchinit` upstream, che l'autore può aggiornare). Verificato: il fit applica il taf a 13 reperti (US 21=1.0, US 76=0.4) e completa.

### English

- **Per-US "Chronology & taphonomy" editor**: the dialog now pre-loads **all the site's US** from `us_table` with two read-only info columns — **Period** and **N. finds** (count from `inventario_materiali` + `pottery`) — so taf can be assigned to every US, not only the dated ones; saved US show their stored values and only filled rows are written. **`taf [0-1]` column** (per-US taphonomic score, 0 = fully disturbed/redeposited, 1 = pristine in situ; an interpretive value, assigned by hand, not computed), persisted in a new `taf` column of `palimpsest_chronology` (idempotent `ALTER TABLE` migration). Loaded/edited/saved like the dates (US without an absolute date are allowed — `start`/`end` optional in *Salva modifiche*); validated to `[0,1]`; handled in CSV import/export. **The AI report uses it**: `SEF_FACTS_R` reads per-US taf and overrides `taf_score` before `fit_sef`, so the estimate down-weights redeposited/disturbed units; the chronology table shown to the agents and in the DOCX/PDF appendix gains a `taf` column. All algorithms honour taf: the three Processing `.rsx` (fit/intrusions/report) now also read per-US taf from `palimpsest_chronology` and override `taf_score` after `read_pyarchinit` — a pyArchInit-local block, so the embedded `.rsx` are no longer byte-identical to palimpsestr 0.22.0 (the clean home remains upstream `read_pyarchinit`). Verified: the fit applies taf to 13 finds (US 21=1.0, US 76=0.4) and completes.
...
- **Motore OxCal permanente**: il driver di calibrazione (`CHRONO_OXCAL_R`) installa OxCal **una sola volta** in una directory stabile (`~/.pyarchinit/oxcal`, sovrascrivibile con la variabile d'ambiente `PYARCHINIT_OXCAL_DIR`) e nelle sessioni successive lo riusa via `oxcAAR::setOxcalExecutablePath()` invece di riscaricarlo nella `tempdir()` ad ogni avvio.
- **Report AI non più troncato** (fix post-tag): il report si interrompeva al limite di 4096 token per risposta. `LLMProviderManager.stream_chat` ora riporta il troncamento (parametro opzionale `meta` → `finish_reason`/`truncated`, retro-compatibile) e il worker del report AI **continua automaticamente** la generazione quando il modello viene tagliato (`finish_reason == length`/`max_tokens`), mantenendo il budget per-risposta al valore universale 4096 (accettato da tutti i modelli) — così il report è completo su qualsiasi provider/modello senza errori di cap. Guard `_supports_meta()` (via `inspect.signature`) per non andare in crash quando QGIS ha ancora in memoria una versione vecchia di `stream_chat` senza `meta` (degrada a singola chiamata).
- **Grafico di calibrazione OxCal**: il driver di calibrazione produce anche un PNG con le curve di probabilità calibrate per US (`plot()` di `oxcAARCalibratedDatesList`); nel dialog cronologia due pulsanti **"Mostra grafico OxCal"** e **"Esporta grafico (PNG)…"** si abilitano dopo "Calibra e salva". **Reso più descrittivo**: pannello per US con curva di probabilità riempita, banda 95% HPD evidenziata, confini HPD, asse calendariale (negativo = a.C.) e titolo con BP±errore + intervallo HPD.
- **Report AI: crash `unsupported format character ','`** (fix post-tag): i prompt usavano `"...%s" % lang_name` ma il testo conteneva `%` letterali (es. "N reperti, %, data media"); passati a concatenazione, niente più `%`-format.
- **Cronologia OxCal: visualizzazione e modifica delle date salvate** (fix post-tag): il dialog non ricaricava i dati già salvati (tabella vuota alla riapertura). Ora la tabella ha 9 colonne (Sito, Area, US, C14 BP, ± err, **start, end**, Lab code, source), **carica all'apertura** le righe esistenti da `palimpsest_chronology` (pulsante "Ricarica dal DB"), permette di **modificare start/end** e salvarle con **"Salva modifiche (start/end)"** (upsert per chiave `(sito,area,us)`), e "Calibra e salva" riscrive start/end nelle celle. Tutti i salvataggi ricaricano la tabella.
- **Report AI: rendering, figure, tabelle, PDF e dati OxCal** (fix post-tag): risolti i problemi del DOCX (markdown grezzo, niente figure, niente tabelle). Nuovo driver `SEF_FACTS_R`: fitta il modello una volta e produce i *fatti* per gli agenti — sintesi per fase, composizione per classe, diagnostiche, **righe di cronologia OxCal** e le figure `gg_*` (generate esplicitamente con `ggsave`, perché `export_sef_report` non lascia la cartella `_figs` quando pandoc/LaTeX riescono). `palimpsest_ai_report.py` ha ora un **renderer Markdown** (titoli, grassetto/corsivo, elenchi puntati/numerati e **tabelle vere**) per **DOCX e PDF**, con le figure incorporate e un'**appendice dati garantita** (tabella per fase + tabella cronologia OxCal). Aggiunto il pulsante **"Salva PDF…"**. Il blocco fatti passato agli agenti ora include le tabelle (sintesi per fase, composizione, cronologia OxCal) così l'AI vede e riporta i dati assoluti; prompt resi più chiari e con richiesta esplicita di tabelle.
...
- **Discovery automatica di pandoc/LaTeX** (`_augment_render_env`): QGIS (app GUI) avvia R con un PATH minimale che di norma **non** include Homebrew (`/opt/homebrew/bin`), MacTeX (`/Library/TeX/texbin`) né TinyTeX → `rmarkdown` non trovava pandoc e il report ripiegava silenziosamente sul solo `.md`; inoltre l'`.rsx` copia il `.md` sull'output `.pdf` dichiarato, producendo un "PDF" che in realtà è markdown (corrotto) e nessun `.docx`. Prima di `processing.run` il dialog ora sonda le posizioni d'installazione note e antepone al `PATH` quelle trovate (+ imposta `RSTUDIO_PANDOC`), così il processo R figlio rende davvero PDF/DOCX.
- **Validazione per contenuto (magic bytes)**: `_show_report` abilita "Apri PDF"/"Apri DOCX" solo se i file sono autentici (PDF inizia con `%PDF`, DOCX è uno zip `PK\x03\x04`) — così il `.pdf` finto (markdown rinominato) non viene mai aperto come PDF, e l'hint "installa pandoc/tinytex" scatta correttamente (prima non scattava perché il `.pdf` finto esisteva).
- **Script R aggiornati per palimpsestr 0.22.0**: i tre `.rsx` ricevono `##Source` e i parametri **opzionali** `##Database_file=optional file` + `##PG_connection=optional string`, con un blocco `use_pg` che legge PostGIS quando è passata una DSN libpq, altrimenti il file SQLite (sourcing corretto delle quote da `pyarchinit_quote`/`quota_usm`; `pottery_table`). I tre `.rsx` embeddati sono byte-identici alle sorgenti `qgis/processing/*.rsx`.
- **PostgreSQL/PostGIS** (`_pg_dsn`/`_db_params`): Fit, Intrusions e Report girano ora anche sulla connessione PG attiva di pyArchInit, non solo su SQLite. La URL SQLAlchemy (`postgresql://user:pwd@host:port/db?sslmode=…`) viene convertita in DSN libpq e passata come `PG_connection`; con PG attivo non compare più l'avviso "connetti un DB SQLite". `_describe_db` ora descrive positivamente la connessione PG.
- **Tabella cronologia assoluta `palimpsest_chronology`** (OxCal): nuovo pulsante **"Cronologia assoluta (OxCal)…"** → dialog `PalimpsestChronologyDialog`. Crea la tabella in modo idempotente sul backend attivo (SQLite `INTEGER PRIMARY KEY` / PostgreSQL `SERIAL`, colonne `"start"`/`"end"` quotate perché `end` è riservata in PG). Due modi di popolarla: **(a)** inserisci date C14 (BP ± errore, lab_code) per US e premi **"Calibra e salva"** → un driver Rscript (`oxcAAR::oxcalCalibrate` + `palimpsestr::chronology_from_oxcal`, `quickSetupOxcal` al primo uso) calcola gli intervalli calendariali (anni BCE negativi) scritti come `start`/`end`; **(b)** **import CSV** di range già calibrati (`sito,area,us,start,end,lab_code,source`). Upsert idempotente per chiave `(sito,area,us)`. `read_pyarchinit()` rileva la tabella e la usa al posto della `datazione` testuale — nessuna modifica agli `.rsx` necessaria.

Verifica: sintassi Python OK (`py_compile`); i 3 `.rsx` embeddati byte-identici alle sorgenti 0.22.0; pipeline R del report testata end-to-end sul DB di esempio `villa_romana_pyarchinit.sqlite` (615 reperti, 54 US) → `.md`+`.pdf`+`.docx`; `_pg_dsn` verificato sul formato conn-string reale; DDL+upsert idempotente testato su SQLite; **integrazione cronologia verificata end-to-end**: popolata `palimpsest_chronology` per 3 US, `read_pyarchinit` passa dalle date testuali (425–450 d.C.) agli intervalli calibrati inseriti (es. US 102 → −290/−90).

### English

**The Palimpsest tab now generates a narrated SEF report (PDF/DOCX) and lets you choose which finds to analyse.** A new "Genera report (PDF/DOCX)" button runs the `r:palimpsestrreport` Processing R algorithm (palimpsestr 0.21.0 — interpretive narrative + all `gg_*` diagnostic plots + tables); its `.rsx` is embedded in `RSX_SCRIPTS` and auto-deployed by `install_scripts()`. An inline read-only results panel shows the narrative by reading the `<base>.md` sidecar the algorithm always writes next to the output (so it works even without LaTeX/pandoc), with Open PDF / Open DOCX / Open folder buttons enabled per the files actually produced. A shared "Source" selector (both/materials/pottery) now drives Fit, Intrusions and Report alike, plus Language (it/en) and Format (both/pdf/docx) combos whose `currentIndex()` maps onto the enum order. Because QGIS launches R with a minimal GUI PATH that hides pandoc/LaTeX (so the report would otherwise fall back to markdown only — and the `.rsx` copies that `.md` onto the declared `.pdf`, yielding a corrupt "PDF" and no DOCX), the dialog now probes common install locations and prepends the ones it finds to PATH (+ sets `RSTUDIO_PANDOC`) before running, so the R child renders real PDF/DOCX; `_show_report` validates files by magic bytes (`%PDF`, `PK\x03\x04`) so a markdown-disguised `.pdf` is never offered and the install hint fires correctly. All three `.rsx` (updated for palimpsestr 0.22.0) gained `##Source` plus the optional `##Database_file`/`##PG_connection` parameters with a `use_pg` branch, and are byte-identical to the `qgis/processing/*.rsx` sources. **PostgreSQL/PostGIS**: `_pg_dsn`/`_db_params` convert pyArchInit's active SQLAlchemy URL to a libpq DSN, so Fit/Intrusions/Report now run against the active PG connection (not only SQLite). **Absolute chronology**: a new "Cronologia assoluta (OxCal)…" button opens a dialog that creates the optional `palimpsest_chronology` table (idempotent, SQLite+PG; `"start"`/`"end"` quoted) and fills it either by calibrating entered C14 dates via an Rscript driver (`oxcAAR` + `chronology_from_oxcal`) or by importing a pre-calibrated CSV — `read_pyarchinit()` then uses it in place of the free-text `datazione`. Verified: `py_compile` OK, byte-identical scripts, report pipeline end-to-end on the example DB; DDL/upsert idempotent on SQLite; and the chronology integration confirmed end-to-end (after populating the table, `read_pyarchinit` switches 3 US from 425–450 CE to the inserted calibrated BCE intervals).
...
- **pip riceveva le spec con apici letterali** (`e5f19372`, solo master): `shlex_quote(package)` dentro `subprocess.run([...])` a lista — le virgolette diventano parte dell'argomento. I vecchi pin `==` sopravvivevano solo perché shlex considera `=` "sicuro"; con i floor `>=` pip riceveva `"'SQLAlchemy-Utils>=0.41.0'"` e usciva con errore (il crash QGIS segnalato su Windows). Spec passata raw, come già sul branch.
- **Dedup dei `.dist-info` stali in `ext_libs/`** (`e5f19372` su master, stesso fix sul branch): dist-info duplicati sopravvissuti agli upgrade (es. `langchain_core-0.3.84` accanto a `0.3.86`) potevano oscurare la versione reale a seconda dell'ordine di `listdir`, facendo richiedere lo stesso install ad ogni avvio (il loop "installa langchain-core>=0.3.85" sul branch). Ora a parità di pacchetto vince la versione più alta; rimossi i dist-info stali locali.
- **Ripristinati i pin esatti `==` su master** (`179fa405`, supersede il punto sotto): i vecchi pin di master erano quelli giusti per quel codice — ripristinati verbatim per tutto lo stack legacy (`SQLAlchemy==1.4.27`, `SQLAlchemy-Utils==0.38.0`, `GeoAlchemy2==0.9.1`, …). Col check `==` l'installer ora **auto-ripara** gli ambienti inquinati: una macchina con SQLAlchemy 2.x / GeoAlchemy2 0.18 (l'ImportError `with_metaclass` visto su Windows) viene riportata alle versioni known-good. I pacchetti con alert Dependabot restano pinnati `==` alle versioni **patchate** (dual-line py3.9/py3.10+ con marker; i due set langchain verificati internamente coerenti su PyPI). Rimossi gli extra solo-branch (psycopg2-binary, qrcode, s3dgraphy, backend remote-storage) mai richiesti dal codice master.
- **SQLAlchemy bloccato sulla linea 1.4 su master** (`0bbbdcb9`): il codice di master usa ancora l'API 1.4 (`engine.execute()`, `autocommit=True`, rimossi in 2.0) in `pyarchinit_db_manager`/`pyarchinit_db_update`/`pyarchinitConfigDialog`/`pyarchinit_pyqgis`; il floor `>=2.0.0` portato dal branch avrebbe fatto aggiornare a 2.x installazioni 1.4.27 funzionanti, rompendo il plugin. Su master ora `SQLAlchemy>=1.4.27,<2.0` (il branch, migrato alla 2.0, resta su `>=2.0.0` — gira con la 2.0.49). Il check del floor nell'installer ora tollera i range con virgola (guard portato anche sul branch).
...
**Fixes the "PyArchInit - Package Installation" dialog broken after the master update.** Porting the hardened `requirements.txt` (patched `>=` floors, security comments, PEP 508 markers) to master had left the old naive set-difference parser in place (whole line vs `name==version`), so every line of the file — blanks, `#`, comment fragments — was reported as a missing package, shown in the table and handed to pip. `PackageManager.check_required_packages` in `__init__.py` is replaced with the `Stratigraph_00001` version: skips comments/blank lines, strips inline comments, evaluates environment markers (keeps only the env-applicable dual-line variant), compares by package name with real version checks (`==` major/minor mismatch, `>=` floor via `packaging.version`), and also scans `ext_libs/` dist-info metadata. Verified standalone against the current `requirements.txt`: 41 clean specs, zero comment/blank leaks, `requests` dual-line correctly resolved to the single applicable variant. **pip received quoted specs** (`e5f19372`, master only): `shlex_quote(package)` inside list-args `subprocess.run` makes the quotes literal argv characters — old `==` pins survived only because shlex treats `=` as safe, while the new `>=` floors made pip fail with non-zero exit (the QGIS crash reported on Windows); the spec is now passed raw as on the branch. **Stale ext_libs dist-info dedup** (`e5f19372` + same fix on the branch): duplicate `.dist-info` dirs surviving upgrades (e.g. `langchain_core-0.3.84` next to `0.3.86`) could shadow the real version depending on `listdir` order, re-asking the same install on every start — the highest version now wins, and the local stale dist-infos were removed. **SQLAlchemy capped to the 1.4 line on master** (`0bbbdcb9`): master's code still uses the 1.4 API (`engine.execute()`, `autocommit=True`, both removed in 2.0), so the ported `>=2.0.0` floor would have upgraded working 1.4.27 installs and broken the plugin — master now requires `SQLAlchemy>=1.4.27,<2.0`, while the 2.0-migrated branch keeps `>=2.0.0`. The installer's floor check now tolerates comma ranges (guard also ported to the branch).
...
**La *Verifica rapporti* ora rileva i paradossi temporali**: quando l'assegnazione di periodo/fase di un'unità contraddice la stratigrafia osservata. La stratigrafia è il dato di riferimento → le auto-correzioni spostano i **periodi**, non i rapporti.

- **Nuovo modulo Qt-free** `modules/utility/temporal_check.py`: legge la cronologia da `periodizzazione_table` (`build_chronology`, archi `cron_iniziale/cron_finale`) e gli intervalli per-US da `us_table` (`load_unit_periods`, perché il projector attacca al grafo solo `periodo_iniziale`), calcola lo `unit_span` per unità (con fallback su fase vuota), poi `detect_temporal` + `solve_fixes`. Backend-agnostico via `DbHandle` (PostgreSQL + SQLite).
- **Tre tipi di paradosso**: `TEMPORAL_INVERSION` (relazione d'ordine tra due unità datate dove la più recente in stratigrafia risulta interamente più **antica** in periodo), `TEMPORAL_CONTEMPORANEITY` (unità dichiarate contemporanee ma con archi `cron` **disgiunti**), `TEMPORAL_UNEVALUABLE` (informativo: una delle due unità non è databile).
- **Confine stretto** (decisione utente, spec §5): "A interamente più antica di B" = `cron_finale(A) < cron_iniziale(B)`. Periodi **adiacenti che si toccano** in un solo punto valgono come sovrapposti → **non** sono un paradosso (benefit-of-the-doubt). Le tre regole (inversione, contemporaneità, `_violated`) usano la **stessa** soglia stretta, così rilevamento e punteggio-conflitti non divergono mai su un confine che tocca.
- **Auto-correzione** (`solve_fixes`, euristica di maggioranza): per un arco in paradosso sposta l'unità **mono-periodo** con più conflitti verso il periodo a **spostamento minimo** che soddisfa tutti i vincoli; colma i vuoti univoci di contemporaneità (copia il periodo del vicino datato sull'unità non databile). **Pareggio / unità multi-periodo (CON, span voluti) / nessun target valido → suggerimento "cosa + come"** localizzato, nessuna scrittura. La scelta del target è più esigente del rilevamento (`_fix_satisfies`): per le relazioni d'ordine un periodo che **tocca** soltanto il vicino non è un fix valido — l'unità spostata dev'essere genuinamente più recente/antica.
- **Integrazione retro-compatibile** in `modules/utility/rapporti_check.py`: `check_rapporti` accetta `chrono`/`unit_periods` opzionali (assenti → blocco temporale saltato, nessun caller esistente rotto); `Edit` guadagna `set_fields`; `apply_edits`/`rollback` scrivono e snapshottano le colonne periodo dietro una whitelist (`_PERIOD_COL_WHITELIST`, anti-injection).
- **GUI** `gui/rapporti_check_dialog.py`: la verifica costruisce cronologia + intervalli e accoda i paradossi al report; l'anteprima mostra le **modifiche di periodo** proposte; l'apply esegue un **auto-backup del DB** (SQLite + `pg_dump` in dir home-relative) prima di scrivere i periodi.

Test: `tests/sync/test_temporal_check.py` + `test_rapporti_check.py` (**39 passed**, inclusi confine-che-tocca nelle due direzioni d'arco, inversione/contemporaneità, gap-fill, snapshot/rollback misto rapporti+periodi). Suite `tests/sync` completa: **490 passed** (SQLite); i fallimenti/errori residui sono i test **PostgreSQL d'ambiente** (server offline / drift schema), non legati alla feature. Baseline AC-2 byte-identica (il check non gira all'export).

### English

**Verifica rapporti now detects temporal paradoxes**: period/phase assignments that contradict the observed stratigraphy. Stratigraphy is the reference → auto-fixes move **periods**, not relations. New Qt-free `modules/utility/temporal_check.py` reads the chronology from `periodizzazione_table` (`build_chronology`) and per-US intervals from `us_table` (`load_unit_periods` — the projector only attaches `periodo_iniziale` to graph nodes), computes each unit's `unit_span` (blank-fase fallback), then runs `detect_temporal` + `solve_fixes` (DbHandle, PG+SQLite). Three kinds: `TEMPORAL_INVERSION` (an order relation where the stratigraphically-later unit is entirely more **ancient** by period), `TEMPORAL_CONTEMPORANEITY` (units declared coeval but with disjoint `cron` spans), `TEMPORAL_UNEVALUABLE` (informational: one unit undatable). **Strict interval boundary** (user decision, spec §5): "A entirely more ancient than B" = `cron_finale(A) < cron_iniziale(B)`; adjacent periods that merely **touch** at one point count as overlapping → not a paradox. Inversion, contemporaneity and `_violated` all use the same strict rule, so detection and conflict-scoring never disagree on a touching boundary. **Auto-correction** (`solve_fixes`, majority heuristic) moves the most-conflicted **mono-period** unit to the minimal-shift valid period, and gap-fills unambiguous contemporaneity (copy the dated neighbour's period to the undatable unit); **tie / multi-period (CON) / no valid target → localized "what + how" suggestion**, no write. Fix-target selection is stricter than detection (`_fix_satisfies`): for ordered relations a period that merely touches the neighbour is not a valid fix. Backward-compatible wiring in `modules/utility/rapporti_check.py` (`check_rapporti` gains optional `chrono`/`unit_periods`; absent → temporal block skipped; `Edit` gains `set_fields`; `apply_edits`/`rollback` write+snapshot period columns behind a whitelist). GUI builds chrono+intervals, previews proposed **period changes**, and auto-backs-up the DB (SQLite + `pg_dump`) before period writes. 39 temporal/rapporti tests pass (incl. touching-boundary both arc directions); full `tests/sync` 490 passed (SQLite; residual failures are environmental PostgreSQL); AC-2 baseline byte-identical.
