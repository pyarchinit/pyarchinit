...
- **Grafico di calibrazione OxCal**: il driver di calibrazione produce anche un PNG con le curve di probabilità calibrate per US (`plot()` di `oxcAARCalibratedDatesList`); nel dialog cronologia due pulsanti **"Mostra grafico OxCal"** e **"Esporta grafico (PNG)…"** si abilitano dopo "Calibra e salva".
