...
class pyarchinit_Struttura(QDialog, Ui_DialogStruttura):
	...
	def generate_list_pdf(self):
		...
		for i in range(len(self.DATA_LIST)):
			...
			sigla_struttura = ('%s%s') % (str(self.DATA_LIST[i].sigla_struttura), str(self.DATA_LIST[i].numero_struttura))
	...
	def fill_fields(self, n=0):
		...
		try:
			...
			str(self.textEdit_descrizione_struttura.setText(self.DATA_LIST[self.rec_num].descrizione))							#6 - descrizione
			str(self.textEdit_interpretazione_struttura.setText(self.DATA_LIST[self.rec_num].interpretazione))				#7 - interpretazione
